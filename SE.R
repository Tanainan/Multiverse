...
all.SEfer <- list()  # all SE size for fertility for all the analyses specified in annlist
all.SErela <- list()
all.SEinter <- list()
...
# compute the proportion of data sets with a significant interaction SEect (p.707)
...
# histograms of SE size
hists <- list()
pv <- list()
ylabs=c("Frequency","Frequency","Frequency","Frequency","Frequency","Frequency")
xlabs=c("SE","SE","SE","SE","SE","SE")
...
for (iii in c(2,4,5,6)){ #in the paper, we only show the grids for analyses 2,4,5, and 6
  ...
  grids[[ann]] <- ggplot(df, aes(x = category1, y = category2, fill = sign)) +
    ...
    scale_fill_manual(values = c(significant = "white", nonsignificant = "white")) + ######### signi = grey
